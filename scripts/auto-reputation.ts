 ...
 *
 * Run every 4 h (add to crontab):
 *   0 */4 * * * cd /path/to/ai-trading-agent-tutorial && \
 *     npx hardhat run scripts/auto-reputation.ts --network sepolia \
 *     >> /tmp/auto-rep.log 2>&1
 */
...
// ── Judge wallet ──────────────────────────────────────────────────────────────
// Deterministic from seed — same address every run.
// This wallet must NOT be the operator of any registered agent.
// ⚠  Fund it with at least 0.01 Sepolia ETH before first run.
const JUDGE_WALLET_SEED = "lablab-hackathon-judge-wallet-v1";

// ── Minimum ETH required in judge wallet ─────────────────────────────────────
...
async function main() {
  ...
  // Judge wallet — detached from hardhat signers, connected to same provider
  ...
  if (judgeBalance < MIN_JUDGE_BALANCE) {
    console.error(
      `\n❌  Judge wallet underfunded.\n` +
      `    Send ≥ 0.01 Sepolia ETH to: ${judgeWallet.address}\n` +
      `    (Faucet: https://sepoliafaucet.com)\n`
    );
    process.exit(1);
  }

  // Contract instances (read = provider, write = judgeWallet)
  ...
  for (let id = 1n; id <= total; id++) {
    ...
    try {
      ...
      // Fetch all metrics in one round-trip
      ...
      process.stdout.write(
        ...
        `val=${String(valScore).padStart(3)} trades=${String(tradeCount).padStart(3)} ` +
        `claimed=${claimed ? "Y" : "N"} → ${score}/100 (prev ${prevRep}) ... `
      );
